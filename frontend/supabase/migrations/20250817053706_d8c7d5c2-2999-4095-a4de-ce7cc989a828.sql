...
-- Rename leads table to campaign_leads for better clarity (only if not already renamed)
DO $$
BEGIN
...
-- Update execution_logs table structure (add missing columns)
...
ADD COLUMN IF NOT EXISTS execution_time timestamp with time zone DEFAULT now();

-- Make action column NOT NULL where it isn't already
UPDATE execution_logs SET action = 'unknown' WHERE action IS NULL;
ALTER TABLE execution_logs ALTER COLUMN action SET NOT NULL;
