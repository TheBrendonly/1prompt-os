-- Create function to properly schedule leads when uploaded
CREATE OR REPLACE FUNCTION public.schedule_campaign_leads(p_campaign_id uuid)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Schedule all unscheduled leads for this campaign
  FOR lead_record IN 
    SELECT id FROM leads 
    WHERE campaign_id = p_campaign_id 
    AND (scheduled_for IS NULL OR scheduled_for = created_at)
    ORDER BY created_at
  LOOP
  ...
  INSERT INTO execution_logs (
    ...
    format('Scheduled %s leads for campaign execution', total_scheduled)
  );
